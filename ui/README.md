...
## 在react组件之外进行跳转操作
这里`umi4`依旧保留了原来的`history` `api`: [history_umi4](https://umijs.org/docs/api/starlite/v1/api#history), 详细的`api`的使用可以看这个: [history API Reference](https://github.com/remix-run/history/blob/main/docs/api-reference.md)
...
# 标题
配置文件中有`title`配置项: [title_umi4](https://umijs.org/docs/api/starlite/v1/config#title), 这个配置的是全局的标题, 每个页面都会使用这个标题, 如果需要动态配置标题, 每个页面不同, 则需要使用`Helmet`: [helmet_umi4](https://umijs.org/docs/api/starlite/v1/api#helmet), 再配合当前页面的`location`信息和接口返回的菜单数据就可以了, 详情可以查看: `/src/components/LayoutWrapper.tsx`
