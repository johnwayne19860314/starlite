...
import (
	...
	"github.startlite.cn/itapp/startlite/pkg/lines/runtime/types"
)
...
func marshalDeepObject(in interface{}, path []string) ([]string, error) {
	...
	case []interface{}:
		...
		for i, iface := range t {
			...
			if err != nil {
				return nil, errorx.Wrap(err, "error traversing array")
			}
	...
	case map[string]interface{}:
		...
		for _, k := range keys {
			...
			if err != nil {
				return nil, errorx.Wrap(err, "error traversing map")
			}
			result = append(result, fields...)
		}
	default:
...
func MarshalDeepObject(i interface{}, paramName string) (string, error) {
	...
	if err != nil {
		return "", errorx.Wrap(err, "failed to marshal input to JSON")
	}
	var i2 interface{}
	...
	if err != nil {
		return "", errorx.Wrap(err, "error traversing JSON structure")
	}
...
func UnmarshalDeepObject(dst interface{}, paramName string, params url.Values) error {
	...
	if err != nil {
		return errorx.Wrap(err, "error assigning value to destination")
	}
...
func fieldIndicesByJsonTag(i interface{}) (map[string]int, error) {
	...
	if t.Kind() != reflect.Struct {
		return nil, errorx.New("expected a struct as input")
	}
...
func assignPathValues(dst interface{}, pathValues fieldOrValue) error {
	...
	case reflect.Slice:
		...
		if err != nil {
			return errorx.Wrap(err, "error assigning slice")
		}
		iv.Set(dstSlice)
		return nil
	case reflect.Struct:
		...
		if it.ConvertibleTo(reflect.TypeOf(types.Date{})) {
			...
			if err != nil {
				return errorx.Wrap(err, "invalid date format")
			}
			dst := iv
		...
		if err != nil {
			return errorx.Wrap(err, "failed enumerating fields")
		}
		for _, fieldName := range sortedFieldOrValueKeys(pathValues.fields) {
			...
			if err != nil {
				return errorx.Wrapf(err, "error assigning field [%s]", fieldName)
			}
		}
		return nil
	case reflect.Ptr:
	...
	case reflect.String:
		...
		return nil
	default:
		return errorx.New("unhandled type: " + it.String())
	}
}
...
func assignSlice(dst reflect.Value, pathValues fieldOrValue) error {
	...
	for i := 0; i < nValues; i++ {
		...
		if !found {
			return errorx.New("array deepObjects must have consecutive indices")
		}
		values[i] = fv.value
	}
	...
	for i := 0; i < nValues; i++ {
		...
		if err != nil {
			return errorx.Wrap(err, "error binding array")
		}
	}
